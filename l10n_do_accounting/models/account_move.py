...
class AccountMove(models.Model):
    ...
    def init(self):  # DO NOT FORWARD PORT
        ...
        if self.env.cr.fetchone()[0] or False:

            _logger.info("Starting data migration from account_invoice to account_move")

            document_type_dict = {
                "01": self.env.ref("l10n_do_accounting.ncf_fiscal_client").id,
                "02": self.env.ref("l10n_do_accounting.ncf_consumer_supplier").id,
                "03": self.env.ref("l10n_do_accounting.ncf_debit_note_client").id,
                "04": self.env.ref("l10n_do_accounting.ncf_credit_note_client").id,
                "11": self.env.ref("l10n_do_accounting.ncf_informal_supplier").id,
                "12": self.env.ref("l10n_do_accounting.ncf_unique_client").id,
                "13": self.env.ref("l10n_do_accounting.ncf_minor_supplier").id,
                "14": self.env.ref("l10n_do_accounting.ncf_special_client").id,
                "15": self.env.ref("l10n_do_accounting.ncf_gov_client").id,
                "16": self.env.ref("l10n_do_accounting.ncf_export_client").id,
                "17": self.env.ref("l10n_do_accounting.ncf_exterior_supplier").id,
            }

            domain = [("country_id", "=", self.env.ref("base.do").id)]
            for company in self.env["res.company"].search(domain):

                # Sale invoices routine
                sales_journal = self.with_context(
                    default_type="out_invoice", default_company_id=company.id
                )._get_default_journal()

                sale_invoices = self.search(
                    [
                ...
                for i, invoice in enumerate(sale_invoices):
                    ...
                    AND state != 'draft';
                    """
                    self.env.cr.execute(query % invoice.name)
                    ...
                    if data:
                        ...
                        document_type_key = data[0][1:3] if len(data[0]) == 11 \
                            else data[0][9:-8]
                        try:
                ...
                purchase_journal = self.with_context(
                    default_type="in_invoice", default_company_id=company.id
                )._get_default_journal()

                purchase_invoices = self.search(
                    [
                ...
                purchase_journal.with_context(allow_documents=True).write(
                    {"l10n_latam_use_documents": True}
                )
