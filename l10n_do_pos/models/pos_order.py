...
class PosOrder(models.Model):
    ...
    ncf_l10n_do_origin_ncf = fields.Char("Affects",)
    ncf_expiration_date = fields.Date(string="NCF expiration date",)
    fiscal_type_id = fields.Many2one("account.fiscal.type", string="Fiscal Type",)
    fiscal_sequence_id = fields.Many2one(
        "account.fiscal.sequence", string="Fiscal Sequence", copy=False,
    )
            ...
            if order.config_id.invoice_journal_id.l10n_do_fiscal_journal:
                for statement in order.statement_ids:
                    ...
                    if statement.journal_id.is_for_credit_notes:
                        ...
                        # (NCF credit note)
                        credit_note_order = self.env["pos.order"].search(
                            [("ncf", "=", statement.note)]
                        )
                        ...
                        if credit_note_order.invoice_id.state == "paid":
                            raise UserError(
                                _(
                                    "The credit note used in another invoice,"
                                    " please unlink that invoice."
                                )
                            )
                        credit_note_order.update({"is_used_in_order": True})
                        ...
                        order._reconcile_refund_invoice(credit_note_order.invoice_id)
                    else:
    ...
    def action_pos_order_invoice_no_return_pdf(self):
        ...
        for order in self:
            ...
            if not order.partner_id:
                ...
                order.write({"partner_id": order.config_id.default_partner_id.id})
            ...
            message = _(
                "This invoice has been created from the point of sale "
    ...
    def create_from_ui(self, orders):
        ...
        for order in order_list:
            ...
            if (
                order_obj.config_id.invoice_journal_id.l10n_do_fiscal_journal
    ...
    def return_from_ui(self, orders):
        ...
        for tmp_order in orders:
            ...
            if (
                returned_order.state == "draft"
                and returned_order.config_id.invoice_journal_id.l10n_do_fiscal_journal
            ):
    ...
    def create_pos_order_refund_invoice(self):
        ...
        if origin_order:
            ...
            if round(refund_invoice.amount_total, -2) != round(
                abs(self.amount_total), -2
            ):
                raise UserError(
                    _(
            ...
            raise UserError(_("Order not found, pleas contact your manager"))

    def get_next_fiscal_sequence(
        self, fiscal_type_id, company_id, mode, lines, uid, payments
    ):
        """
        ...
        if not fiscal_sequence:
            raise UserError(
                _(
                    u"There is no current active NCF of {}"
                    u", please create a new fiscal sequence "
                    u"of type {}."
                ).format(
                    fiscal_type.name, fiscal_type.name,
                )
            )
    ...
    def confirm_return_order_is_correct(self, uid, lines):
        ...
        for order_line in lines:
            if order_line[2]["product_id"] in products_in_order:
                products_in_order[order_line[2]["product_id"]] += order_line[2]["qty"]
            else:
                products_in_order[order_line[2]["product_id"]] = order_line[2]["qty"]
