...
class AccountInvoice(models.Model):
    ...
    def _compute_sequence_almost_depleted(self):
        for invoice in self:
            if (
                invoice.journal_id.ncf_control
                and invoice.type == "out_invoice"
                and invoice.sale_fiscal_type
            ):
            ...
            if (
                invoice.journal_id.purchase_type in ("informal", "minor", "exterior")
                and invoice.type == "in_invoice"
                and invoice.journal_id.purchase_type
            ):
                sequence = invoice.journal_id.date_range_ids.filtered(
    ...
    ncf_expiration_date = fields.Date(
        "Válido hasta", compute="_compute_ncf_expiration_date", store=True
    )
    ...
    def onchange_journal_id(self):
        ...
        if self.journal_id.type == "purchase":
            ...
            if self.partner_id.id == self.company_id.partner_id.id:
                ...
                if not journal_id:
                    raise ValidationError(
                        _("No existe un Diario de Gastos Menores," " debe crear uno.")
                    )
        ...
        for inv in self:
            ...
            if inv.type == "out_invoice" and inv.journal_id.ncf_control:
                ...
                if sequence.number_next_actual > sequence.max_number_next:
                    raise ValidationError(
                        _(
                            ...
                            " contacte al responsable de contabilidad ({})."
                        ).format(
                            dict(self._fields["sale_fiscal_type"].selection).get(
                                self.sale_fiscal_type
                            ),
                            sequence.max_number_next,
                        )
                    )

                if (
                    inv.sale_fiscal_type in ("fiscal", "gov", "special")
                ...
                if (
                    ...
                    and inv.sale_fiscal_type != "unico"
                    and not inv.partner_id.vat
                ):
                    raise UserError(
                        _(
                ...
                raise ValidationError(
                    _(
                        "Para poder emitir una NC mayor a "
    ...
    def create(self, vals):
        if vals.get("sale_fiscal_type", None) == "fiscal":
            ...
            if partner_id and vat and vat.isdigit():
                if len(vat) not in [9, 11] or not (
                    rnc.is_valid(vat) or cedula.is_valid(vat)
                ):
                    raise ValidationError(
                        _(
