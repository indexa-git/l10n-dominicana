...
class IrSequence(models.Model):
    ...
    def _next(self):
        ...
        if sale_fiscal_type:
            ...
            seq_date = self.env['ir.sequence.date_range'].search([('sale_fiscal_type', '=', sale_fiscal_type),
                                                                  ('sequence_id', '=', self.id),
                                                                  ('date_from', '<=', dt),
                                                                  ('date_to', '>=', dt)], limit=1)
            if not seq_date:
                raise ValidationError('No tiene Comprobantes validos para la fecha %s del tipo %s' % (dt,sale_fiscal_type))
            print seq_date.date_to
            print self._context.get('invoice').ncf_date_to
            self._context.get('invoice').ncf_date_to = seq_date.date_to
            print self._context.get('invoice').ncf_date_to
