...
class IrSequence(models.Model):
    ...
    ncf_padding = fields.Integer(
        required=True, default=8, help="Padding legally use by NCF sequences"
    )
    ...
    def get_next_char(self, number_next):
        ...
        if sale_fiscal_type:
            return (
                "B"
                + self.ncf_dict[sale_fiscal_type]
                + "%%0%sd" % self.ncf_padding % number_next
            )
...
class IrSequenceDateRange(models.Model):
    ...
    sale_fiscal_type = fields.Selection(
        "get_sale_fiscal_type_from_partner", string="NCF para"
    )
