    ...
    def _compute_sale_fiscal_type(self):
        ...
        for partner in self:
            ...
            if partner.country_id and not is_dominican_partner:
                ...
                not partner.sale_fiscal_type or partner.sale_fiscal_type == "final"
            ):
    ...
    def rewrite_due_date(self):
        for rec in self:
            ...
            for inv in invoice_ids:
                pterm = (
                    rec.property_payment_term_id
                    or rec.property_supplier_payment_term_id
                )
                if pterm:
                    pterm_list = pterm.with_context(
                        currency_id=inv.company_id.currency_id.id
                    ).compute(value=1, date_ref=inv.date_invoice)[0]
