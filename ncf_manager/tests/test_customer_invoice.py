...
class InvoiceNCFSequenceTest(TransactionCase):
    def setUp(self):
        ...
        self.journal = self.env["account.journal"].create(
            {"name": "Sale Journal", "type": "sale", "ncf_control": True, "code": "SJ"}
        )
        ...
        self.sale_fiscal_type = [
            t[0] for t in self.env["res.partner"]._fields["sale_fiscal_type"].selection
        ]
    ...
    def test_journal(self):
        ...
        self.assertTrue(
            self.journal.ncf_control, "Journal has not ncf_control activated"
        )
        ...
        self.assertEquals(
            len(
                [
                    x
                    for x in sequence_id.date_range_ids
    ...
    def test_fiscal_invoices(self):
        ...
        for i in range(n):
            ...
            self.assertEquals(
                int(str(invoice_id.reference)[3:]), date_range_id.number_next - 1
            )
