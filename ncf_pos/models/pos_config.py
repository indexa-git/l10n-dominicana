...
class PosConfig(models.Model):
    ...
    number_of_days = fields.Integer(string=u"Cantidad de Días Anteriores", default=10)
    order_search_criteria = fields.Many2many(
        "pos.search_criteria", string=u"Criterios de Búsqueda"
    )
    ...
    def onchange_module_account(self):
        default_partner = self.env.ref(
            "ncf_pos.default_partner_on_pos", raise_if_not_found=False
        )
