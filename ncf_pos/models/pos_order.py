...
class PosOrder(models.Model):
    ...
    def _process_order(self, pos_order):
        ...
        for st in pos_order['statement_ids']:
            statement = st[2]
            payment_reference = payment_reference or statement.get('payment_reference', False)
            if statement['journal_id'] == 10001 and statement['note']:
                invoice = self.env['account.invoice'].search([('reference', '=', statement['note'])])
                acc_move_line_ids = order.refund_payment_account_move_line_ids.filtered(lambda p: p.ref == statement['note'])
                invoice.write({
