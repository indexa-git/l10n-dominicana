...
class PosConfiguration(models.TransientModel):
    ...
    pos_session_concile_type = fields.Selection(
        [('session', u'Por session'),
         ('none', u'Manual')],
        string=u"Conciliación", default=u"session"
    )
    pos_session_picking_on_cron = fields.Boolean("Crear conduce desde un cron",
                                                 default=False)
        ...
        try:
            return {
                'pos_session_concile_type': safe_eval(
                    IrConfigParam.get_param('ncf_pos.pos_session_concile_type',
                                            'session'))
                }
        except:
            return {
