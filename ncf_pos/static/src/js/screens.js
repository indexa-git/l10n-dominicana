...
odoo.define("ncf_pos.screens", function (require) {
  ...
  screens.ClientListScreenWidget.include({
    display_client_details: function (visibility, partner, clickpos) {
      ...
      name_input.autocomplete({
        ...
        response: function (event, ui) {
          ...
          if (
            Array.isArray(ui.content) &&
            ui.content.length == 1 &&
            $.isNumeric(name_input.val())
          ) {
            var input = $(this);
      ...
      sale_fiscal_type_ddl.change(function () {
        ...
        if (len == 9 && sale_fiscal_type_vat.rnc.indexOf(this.value) == -1) {
          ...
          len == 11 &&
          sale_fiscal_type_vat.ced.indexOf(this.value) == -1
        ) {
    ...
    save_client_details: function (partner) {
      ...
      if (
        this.pos.sale_fiscal_type_vat.no_vat.indexOf(sale_fiscal_type) === -1 &&
        !rnc
      ) {
      ...
      if (fieldsRequired.length > 0) {
        ...
        this.gui.show_popup("error", {
          ...
          body:
            _t("You must fill in the required fields:") +
            "\n" +
            fields.join(" "),
          cancel: function () {
    ...
    toggle_save_button: function () {
      ...
      if (
        $button &&
        this.pos.config.module_account === true &&
  ...
  var InvoicesListScreenWidget = screens.ScreenWidget.extend({
      ...
      if ($.trim(query) === "") {
        ...
        _.each(allOrders, function (order) {
          _.each(search_criteria, function (criteria) {
            if (order[criteria]) {
              // The property partner_id in order object is an Array, the value to
              // compare is in index 1
              if (_.isArray(order[criteria])) {
                ...
                order[criteria].toLowerCase().indexOf(query.toLowerCase()) > -1
              ) {
                filteredOrders.push(order);
                return true;
              }
    ...
    render_list: function (orders) {
      ...
      orders.forEach(function (order) {
        contents.append(
          QWeb.render("InvoicesLine", { widget: self, order: order })
        );
      });
    },
    close: function () {
          ...
          if (allow_return) {
            order.lines.forEach(function (line_id) {
              ...
              if (product == null) {
                ...
                message =
                  "Algun(os) producto(s) de esta orden no esta(n) disponible(s) en el Punto de Venta, desea devolver los productos restantes?";
            ...
            if (original_orderlines.length == 0) {
              self.gui.show_popup("alert", {
                ...
                body: _t(
                  "No quedan productos retornables en esta orden. Tal vez los productos son No Retornables o no estan disponibles en el Punto de Venta!!"
                ),
              });
  ...
  var OrderRefundPopup = popups.extend({
    ...
    click_complete_return: function () {
      $.each($(".return_qty"), function (index, value) {
        var line_quantity_remaining = parseFloat(
          $(value).find("input").attr("line-qty-remaining")
        );
    ...
    click_return_order: function () {
      ...
      $.each(all, function (index, value) {
        ...
        var line_quantity_remaining = parseFloat(
          input_element.attr("line-qty-remaining")
        );
        ...
        if (
          !$.isNumeric(qty_input) ||
          qty_input > line_quantity_remaining ||
          qty_input < 0
        ) {
        ...
        if (
          qty_input == 0 &&
          line_quantity_remaining != 0 &&
          !self.options.is_partial_return
        ) {
          ...
          if (
            line_quantity_remaining != qty_input &&
            !self.options.is_partial_return
          ) {
            self.options.is_partial_return = true;
          }
    ...
    create_return_order: function (return_lines) {
      ...
      if (self.options.mode == "edit") {
        ...
        refund_order.set_client(
          self.pos.db.get_partner_by_id(order.partner_id[0])
        );
      }
      ...
      Object.keys(return_lines).forEach(function (line_id) {
        ...
        var unit_price_with_discounts = apply_discounts(
          line.price_unit,
          line.discount
        );
        var unit_price_with_taxes = line.qty
          ? parseFloat(line.price_subtotal_incl) / line.qty
          : 0;
  ...
  popups.include({
    ...
    renderElement: function () {
      ...
      if (
        ["TextInputPopupWidget", "TextAreaPopupWidget"].indexOf(this.template) >
        -1
      ) {
        var self = this;
  ...
  screens.PaymentScreenWidget.include({
    init: function (parent, options) {
            ...
            rpc
              ...
              .then(function (result) {
                ...
                if (result.id === false) {
                  ...
                  !client ||
                  (client && client.id !== result.partner_id)
                ) {
                  ...
                  if (paymentline) {
                    msg_error =
                      "Esta Nota de Credito ya esta aplicada a la Orden";
                  } else {
      ...
      for (var n in this.pos.cashregisters) {
        ...
        if (currentCashRegister.journal.id == 10001) {
          ...
          currentCashRegister.journal.payment_form === "card" &&
          !currentCashRegister.credit
        ) {
    ...
    show: function () {
      ...
      var paymentContents = this.$(
        ".left-content, .right-content, .back, .next"
      );
      ...
      if (order && order.is_return_order) {
        ...
        refundConfirm.empty();
        refundConfirm.append(
          QWeb.render("OrderRefundConfirm", { widget: this, order: order })
        );
        if (order.paymentlines.length == 0) {
          ...
          for (var n in this.pos.cashregisters) {
            if (
              this.pos.cashregisters[n].journal.type.toLowerCase() == "cash"
            ) {
        ...
        if (orderValidationDate && now - orderValidationDate <= 5000) {
          ...
          if (!client && invoicing) {
            popupErrorOptions = {
              ...
              body:
                "Debe seleccionar un cliente para poder realizar el pago, o " +
            ...
            if (
              ["fiscal", "gov", "special"].indexOf(client.sale_fiscal_type) > -1
            ) {
              popupErrorOptions = {
                title: "Para el tipo de comprobante",
                body:
                  "No puede crear una factura con crédito fiscal si el cliente " +
              ...
              popupErrorOptions = {
                ...
                body:
                  "El cliente debe tener una cedula si el total de la factura " +
          ...
          if (popupErrorOptions) {
            ...
            this.get_next_ncf(order)
              .done(function () {
                if (
                  order.ncf === "max_ncf_number_reached" ||
                  order.max_ncf_number_reached
                ) {
                  order.max_ncf_number_reached = true;
                  self.gui.show_popup("error", {
                    title: "Limite Máximo para Secuencia de NCF Excedido",
                    body:
                      "Se a alcanzado el limite maximo para el tipo de NCF seleccionado: " +
                      order.get_client().sale_fiscal_type +
              ...
              .fail(function () {
                self.gui.show_popup("error", {
                  ...
                  body:
                    "Puede que haya intermitencia en la conexión a internet " +
      ...
      if (this.pos.get_order().get_due() <= 0) {
        ...
        for (var i = 0; i < this.pos.cashregisters.length; i++) {
          ...
          if (cashregister.journal_id[0] === id && cashregister.popup_options) {
            var popup_options = _.extend(_.clone(cashregister.popup_options), {
              cashregister: cashregister,
            });
            this.gui.show_popup(
              popup_options.popup_name || "alert",
              popup_options
            );
            return false;
          }
    ...
    payment_input: function (input) {
      ...
      if (
        ...
        order.selected_paymentline.cashregister.id === 10001
      ) {
        return false;
      }
      this._super(input);
    ...
    customer_changed: function () {
      ...
      this._super.apply(this, arguments);
      this.$(".sale_fiscal_type_label").text(
        clientFiscalType
  ...
  gui.Gui.include({
    ...
    __disable_keyboard_handler: function () {
      ...
      window.document.body.removeEventListener(
        "keypress",
        current_screen.keyboard_handler
      );
      window.document.body.removeEventListener(
        "keydown",
        current_screen.keyboard_keydown_handler
      );
    },
